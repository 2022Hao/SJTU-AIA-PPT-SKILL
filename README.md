...
一个适配 Codex 与本地终端 Agent 的网页 PPT 技能,用于生成**单文件 HTML 横向翻页 PPT**、PPT 配图、多平台封面,并可同步到 HTMLSlide 网站工作台。
...
> 来源致谢:本项目基于 MIT 协议的 `guizang-ppt-skill` 改造,保留其 HTML PPT 模板、布局经验与检查清单,并面向 SJTU AIA / HTMLSlide 工作流增加网站同步能力。原始版权声明见 `LICENSE`。
...
如果用户访问的 HTMLSlide 网站是 `https://html.inherit-ai.top/`,可以先把本地 Agent 绑定到网站账号:

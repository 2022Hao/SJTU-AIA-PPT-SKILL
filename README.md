# SJTU-AIA PPT Skill · 网页 PPT / 配图 / 封面
...
> **开发主体**: 本项目由**上海交通大学 Agent 创新协会（SJTU AIA）**开发与维护，用于让 AI Agent 生成、修改、管理可演示的 HTML PPT，并与协会的 HTMLSlide 在线工作台联动。

## PPT 已死，HTML 当立！

在 AI 时代，HTML 的代码构建特性几乎是天生的演示利器：AI 可以快速生成绚丽、带流畅动画、能直接打开演示的 HTML slides。

但问题也很现实：代码很炫，人不会改怎么办？数据想修正、文案要微调、图片要替换，难道只能回头继续让 AI 猜？

**上交 Agent 创新协会 HTMLSlide + 本 Skill** 就是为这个工作流准备的：

- **本 Skill**: 让 Codex / 本地终端 Agent 生成单文件 HTML 横向翻页 PPT、PPT 配图、多平台封面，并能读取、修改、同步 HTML PPT。
...
- **Style A: 电子杂志 × 电子墨水**。适合叙事、观点、分享、个人风格表达。
...
安装到 Codex skill 目录后,可以直接对 Agent 说:
...
- 🔄 **HTMLSlide 联动**:本地 Agent 生成或修改 `index.html` 后,可自动同步到 HTMLSlide 网站工作台继续预览、微调和打包
...
把本目录放到 Codex 的 skills 目录中,目录名保持为 `SJTU-AIA-ppt-skill`。安装后确认目录内存在 `SKILL.md`、`assets/`、`references/` 和 `scripts/`。
...
7. **预览** — 浏览器直接打开
8. **可选同步** — 用 `scripts/sync-htmlslide.mjs` 推送到 HTMLSlide 网站工作台
9. **迭代** — inline style 改字号/高度/间距
...
脚本会输出网站授权链接和授权码。用户在 HTMLSlide 网站登录后确认授权，Agent token 会保存到 `~/.htmlslide-agent/config.json`。后续 `push` / `pull` / `list` 都会使用这个授权，不需要在命令行里输入网站密码。

新建或覆盖网站项目:

```bash
node scripts/sync-htmlslide.mjs push path/to/ppt/index.html --title "项目标题"
```
...
node scripts/sync-htmlslide.mjs pull --title "网站里的 PPT 标题" --out path/to/ppt/index.html
# 修改 index.html 后
node scripts/sync-htmlslide.mjs push path/to/ppt/index.html
```

同步脚本会在 `index.html` 同级生成 `.htmlslide-sync.json`,保存 `projectId`。后续再次运行会更新网站里的同一个项目,不会重复创建。脚本会把本地图片/字体内联进 HTML,所以网站 iframe 预览时不需要访问本地 `images/` 路径。
...
│   └── sync-htmlslide.mjs      ← 同步本地 HTML PPT 到 HTMLSlide 网站
└── references/
...
本项目改造自 MIT 协议的 `guizang-ppt-skill`。原始版权声明保留在 `LICENSE` 中。

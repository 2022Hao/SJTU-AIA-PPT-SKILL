...
HTMLSlide 默认站点已经固定为 `https://html.inherit-ai.top`，API 默认是 `https://html.inherit-ai.top/api`。除非用户明确改地址，不要再猜端口、不要再试 `:8085`。
...
- 如果历史配置里还残留 `https://html.inherit-ai.top:8085`，脚本会自动迁移到 `https://html.inherit-ai.top`
